...
def prepare_img_input_in_storage_format(request, name, image, resize_to_shape):
    npimg = np.fromfile(image, np.uint8)
    img = cv2.imdecode(npimg, cv2.IMREAD_COLOR).astype(np.float32)
    img = cv2.resize(img, (resize_to_shape[1], resize_to_shape[0]))
    target_shape = (img.shape[0], img.shape[1])
    img = img.reshape(1,target_shape[0],target_shape[1],3)
    request.inputs[name].CopyFrom(make_tensor_proto(img, shape=img.shape))
...
def update_people_coordinate(output_nd, people):
    for i in range(output_nd.shape[0]):
        if len(people) < i + 1:
            people.append({'coordinate': output_nd[i,0,:].tolist()})
        else:
            people[i].update({'coordinate': output_nd[i,0,:].tolist()})
    return people

@app.route('/', methods=["GET","POST"])
def predict():
        if not flask_request.method == "POST":
            return

        if flask_request.files.get("image"):
            image_file = flask_request.files["image"]


        args = {'grpc_address': 'localhost', 'grpc_port': 9000,\
        'pipeline_name': 'find_face_images', 'image_input_name': 'image',\
        'image_input_path': image_file, 'face_images_output_name': 'face_images',\
        'face_images_save_path': '', 'image_width': 600, 'image_height': 400,\
            'input_image_layout': 'STORAGE', 'frame': None}
        ...
        elif args['input_image_layout'] == 'STORAGE':
            prepare_img_input_in_storage_format(request, args['image_input_name'], args['image_input_path'], (int(args['image_height']), int(args['image_width'])))
        else:
